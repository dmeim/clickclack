...
// Reusing the User type definition
type User = {
  id: string;
  name: string;
  stats: {
...
export default function UserHostCard({
  ...
  if (
    ...
    const typed = user.stats?.wordsTyped || 0;
    ...
    const currentStepProgress = user.stats?.progress || 0;
    ...
    return (
      ...
      >
        <div className="flex items-center gap-4 w-1/3">
          ...
          <div className="font-bold text-lg truncate select-none">
            {user.name}
          </div>
          <div className="text-xs opacity-70 font-mono select-none">
            {user.id.slice(0, 4)}
          </div>
        </div>
        ...
        <div className="flex gap-4 w-1/3 justify-end items-center">
          ...
          <div className="text-right w-16">
            <div className="text-xl font-bold opacity-80">
              {Math.round(user.stats?.accuracy || 0)}%
            </div>
            <div
              className="text-xs"
              style={{ color: GLOBAL_COLORS.text.secondary }}
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start cursor-grab">
        <div
          className="font-bold text-[1.2em] truncate w-3/4 select-none"
          title={user.name}
        >
          {user.name}
        </div>
        <div className="text-[0.6em] opacity-70 font-mono mt-1 select-none">
          {user.id.slice(0, 4)}
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div
          className="flex justify-between text-[0.8em]"
          style={{ color: GLOBAL_COLORS.text.secondary }}
        >
          <span>Progress</span>
      ...
      <div className="grid grid-cols-2 gap-4 mt-2">
        <div className="bg-gray-800 rounded p-3 text-center">
          <div
            className="text-[1.5em] font-bold leading-none"
            style={{ color: theme.buttonSelected }}
          >
            {Math.round(user.stats?.wpm || 0)}
          </div>
          <div
            className="text-[0.6em] mt-1"
            style={{ color: GLOBAL_COLORS.text.secondary }}
          >
            WPM
          </div>
        ...
        <div className="bg-gray-800 rounded p-3 text-center">
          <div className="text-[1.5em] font-bold opacity-80 leading-none">
            {Math.round(user.stats?.accuracy || 0)}%
          </div>
          <div
            className="text-[0.6em] mt-1"
